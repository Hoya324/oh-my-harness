...
두 런타임은 동일한 independence 규칙을 지킵니다. 생성자가 자신의 작업을 판정해서는 안 됩니다. Claude Code는 새로운 네이티브 서브에이전트 또는 명확히 다른 외부 검증자를 사용할 수 있습니다. Codex는 새 Codex 서브에이전트 또는 선택 모델/런타임이 생성자와 명확히 다를 때만 `codex exec -s read-only`를 사용할 수 있습니다. 구별되는 판정자가 없으면 검증 범위 축소를 보고하고 결과를 independent라고 부르지 않습니다.

외부 검증자는 read-only로 진단하고, 생성 워크플로우가 수정을 적용합니다. 각 라운드는 새 컨텍스트에서 시작하며 앞선 결론을 전달받지 않습니다. 선택적 CLI가 없으면 우아하게 축소됩니다. 둘 이상의 독립 모델이 보고한 finding은 두 런타임 모두에서 고신뢰 합의 신호입니다.

Codex 네이티브 `/omh-verify`는 동일한 diff와 설정을 읽고, 같은 메모리 저장소에 공유 finding을 기록하며, read-only 경계를 약화하지 않고 quick/standard/architect 역할 의미를 사용합니다.
