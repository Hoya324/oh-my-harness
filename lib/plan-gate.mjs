 ...
 * @param {{toolName:string, gatedTools?:string[], maxDenials?:number, featureOff?:boolean, disabled?:boolean, isPlanFile?:boolean}} signals
...
export function evaluatePlanGate(state, signals) {
  ...
  // Writing the native plan-mode plan file is HOW the model satisfies this gate
  // (plan mode requires writing the plan before ExitPlanMode). Never gate it,
  // and never count it toward the denial cap.
  if (s.isPlanFile) return { action: 'allow', reason: '', stopCause: 'plan_file', nextState: state };
